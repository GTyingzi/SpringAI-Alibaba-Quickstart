package com.yingzi.rag.service.impl;
...
import com.yingzi.rag.service.RagService;
 ...
 */
@Service
public class LocalRagServiceImpl implements RagService {


    private static final Logger logger = LoggerFactory.getLogger(LocalRagServiceImpl.class);
    ...
    public LocalRagServiceImpl(ChatModel chatModel, VectorStore vectorStore, RerankModel rerankModel,
    ...
    public void importDocument() {
        // 1. parse document
        PagePdfDocumentReader reader = new PagePdfDocumentReader(springAiResource);
        List<Document> documents = reader.get();
        logger.info("Read {} documents", documents.size());

        // 2. split trunks
        List<Document> splitDocuments = new TokenTextSplitter().apply(documents);
        logger.info("Split {} documents into {} trunks", documents.size(), splitDocuments.size());
